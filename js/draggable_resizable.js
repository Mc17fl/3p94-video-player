// Code Taken From https://codepen.io/QDeltaE/pen/VWEyLj with minmial adjustments (from user @QDeltaE).
// All public code from codepen.io is under MIT License and is therefore allowed for use.
// This code is for the dragging and resizing of the editing tabs
...
.on(mousedownEventType, function(e){
  if (!e.target.classList.contains("resize") && !resizing && !e.target.classList.contains("disp") && !e.target.classList.contains("source-count")&& !e.target.classList.contains("delete-source") && !e.target.classList.contains("scene-name") && !e.target.classList.contains("fa-window-close") && !e.target.classList.contains("preview-toolbar-icon")) {
    currentDragged = $(this);
    dragging = true;
    sX = e.pageX;
    sY = e.pageY;
    ...
    $("#audio-sortable").sortable({
      revert: true
    }); 
    $("#assets-sortable").sortable({
      revert: true
    }); 
  }
});
